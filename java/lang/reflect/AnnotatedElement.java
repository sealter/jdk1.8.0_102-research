...
/*
   Represents an annotated element of the program currently running in this VM.
     ...
     这句话再次说明了抽象表现为Interface ? （ 策略模式？）

     想要的是面条， 但是我定义面粉，并提供一种方法将面粉做成面条。 由于抽象定义了面粉，我不仅仅可以做面条，也可以做包子。
     ...
     既是分解又是聚合：
      ...
      抽象属性由行为（interface）表示？

     一碗面条： 盐、青菜、鸡蛋、胡椒、味精
     另一碗面条： 不要味精
        装饰者模式？ 区别？

      业务产品与框架， 把 业务产品代码当作框架写，以期能适应多数场景，比如：
          旅游品类， 目前三级品类， 框架级别的代码是与DB无关
